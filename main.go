...
func (t Training) meanSpeed() float64 {
	meanSpeedKmH := t.distance() / t.Duration.Hours()
	return meanSpeedKmH
}
...
func (r Running) Calories() float64 {
	caloriesCount := ((CaloriesMeanSpeedMultiplier*r.meanSpeed() + CaloriesMeanSpeedShift) *
...
func (w Walking) Calories() float64 {
	...
	caloriesCount := (CaloriesWeightMultiplier*w.Weight + (math.Pow(w.meanSpeed()*KmHInMsec, 2)/(w.Height/CmInM))*
...
func (s Swimming) meanSpeed() float64 {
	// вставьте ваш код ниже
	meanSpeedKmH := float64(s.LengthPool) * float64(s.CountPool) / MInKm / s.Duration.Hours()
	return meanSpeedKmH
}
